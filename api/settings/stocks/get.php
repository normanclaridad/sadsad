...
$columns = [  'sp.name', 'p.name', 'st.status', 'st.created_at' ];
...
if( !empty($params['search']['value']) ) {
    ...
    $whereCondition .= " OR sp.name LIKE '%".$params['search']['value']."%' )";
...
//Get total
$totalRecords = $stocks->getTotal($whereCondition, $sortBy);

//Get all tsag
$fetchResults = $stocks->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($fetchResults AS $row) {
    ...
    $action = '<a class="btn btn-circle btn-sm btn-primary btn-edit" data-id="'. $row['id'] .'"data-supplier-id="'. $row['supplier_id'].'" data-product-id="'. $row['product_id'] .'" data-unit-id="'. $row['unit_id'] .'" data-quantity="'. $quantity .'" data-status="'. $row['status'] .'"><i class="bx bx-pencil"></i></a>';
