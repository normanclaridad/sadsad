...
$columns = [  'ur.name', 'ur.status', 'ur.created_at' ];

$whereCondition = $sqlTot = $sqlRec = '';

if( !empty($params['search']['value']) ) {
    $whereCondition .= " AND ";
    $whereCondition .= " ( ur.name LIKE '%". $params['search']['value'] ."%' )";    
...
//Get total
$totalRecords = $userRoles->getTotal($whereCondition, $sortBy);

//Get all tsag
$fetchResults = $userRoles->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
...
$baseUrl = $protocol . $_SERVER['HTTP_HOST'] . '/views/settings/user-roles/'.$btnAction.'.php?' . $urlFormer;
