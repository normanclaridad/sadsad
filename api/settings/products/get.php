...
//Get total
$totalRecords = $products->getTotal($whereCondition, $sortBy);

//Get all tsag
$fetchResults = $products->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($fetchResults AS $row) {
    ...
    $action .= '&nbsp; <a class="btn btn-circle btn-sm btn-danger btn-delete" data-id="'. $row['id'] .'" data-name="'. $row['name'] .'" data-description="'. $row['description'] .'" data-status="'. $row['status'] .'"><i class="bx bxs-trash"></i></a>';
